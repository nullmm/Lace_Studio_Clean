...
# 强制关闭可能导致 120GB 显存分配的 cuDNN 算法搜索
...
class VQVAE(nn.Module):
    ...
    def forward(self, inp, ret_usages=False):   # -> rec_B3HW, idx_N, loss
        ...
        f_hat, usages, vq_loss = self.quantize(f.float(), ret_usages=ret_usages)
    ...
    def img_to_idxBl(self, inp_img_no_grad: torch.Tensor, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None) -> List[torch.LongTensor]:    # return List[Bl]
        ...
        # 1. 在半精度保护伞下完成极吃显存的编码运算
        with torch.amp.autocast('cuda', enabled=True):
            f = self.quant_conv(self.encoder(inp_img_no_grad))
            
        # 2. 走出保护伞，强转回 .float() 与密码本的 FP32 对齐！彻底消除类型冲突！
        res = self.quantize.f_to_idxBl_or_fhat(f.float(), to_fhat=False, v_patch_nums=v_patch_nums)
        return res
    ...
    def embed_to_img(self, ms_h_BChw: List[torch.Tensor], all_to_max_scale: bool, last_one=False) -> Union[List[torch.Tensor], torch.Tensor]:
        f_hats = self.quantize.embed_to_fhat(ms_h_BChw, all_to_max_scale=all_to_max_scale, last_one=last_one)
        
        with torch.amp.autocast('cuda', enabled=True):
            if last_one:
                return self.decoder(self.post_quant_conv(f_hats)).clamp_(-1, 1)
            else:
    ...
    def img_to_reconstructed_img(self, x, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None, last_one=False) -> List[torch.Tensor]:
        ...
        ls_f_hat_BChw = self.quantize.f_to_idxBl_or_fhat(f.float(), to_fhat=True, v_patch_nums=v_patch_nums)
